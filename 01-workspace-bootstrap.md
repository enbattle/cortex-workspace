...
### `README.md`
Human-facing quickstart. Sections: what this workspace is (3–4 sentences); prerequisites; setup (`clone → ./scripts/bootstrap.sh → ./scripts/adapt.sh → open in your agent tool`); the workflow at a glance (spec-new → spec-clarify → implement → review → retro, one line each), including the universal invocation phrasing from R8; a **"what this workspace believes"** section — eight to ten one- or two-sentence summaries of the load-bearing practices (spec-first with the clarify gate, locked failing tests before implementation, isolated adversarial review, specs travel with code, artifacts carry state, budgeted loops with human escalation, the constitution, untrusted content as data, routed context, evidence-driven change), each citing its design rule or command, with a note that the section is non-normative and canonical sources win on any conflict; where things live (harness vs knowledge vs repos, with the R3 rule stated plainly); supported agent tools and what `adapt.sh` generates for each; how to add a new repo (edit `repos.yaml`, re-run bootstrap and adapt). This README is the single front door for orientation — the generated workspace ships no separate highlights file. Keep it under 120 lines.
...
- [ ] `README.md` contains the non-normative "what this workspace believes" section with citations to design rules and commands, and no separate highlights file exists in the workspace.
