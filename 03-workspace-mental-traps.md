...
This document exists to help anyone building or maintaining an AI workspace avoid re-learning, the expensive way, lessons that have already been paid for. It is the "why" layer of the set: the bootstrap document describes what to build, the extensions document describes what to build later, and this one describes how to *think* while deciding. It is written for humans making design decisions. Agents are best pointed at it only when explicitly asked to evaluate or change the workspace's structure, never during routine execution — loading philosophy into an implementation session is itself one of the traps described below (T8).
